...
public class LogListenerService extends LogListenerAdapter {
  ...
  public LogListenerStatusResponseDto getStatus() {
    ...
    return LogListenerStatusResponseDto.builder()
        .status(
            isServiceRunning
                ? LogListenerStatusResponseDto.LogListenerStatus.STARTED
  ...
  public void handle(String line) {
    ...
    for (LogListenerRegexResponseDto regex : regexList) {
      ...
      if (matcher.matches()) {
        ...
        ipLogMap
            .get(ip)
            .removeIf(timestamp -> Duration.between(timestamp, now).compareTo(TIME_WINDOW) > 0);
    ...
    switch (findTimeTypeSetting.value()) {
      ...
      default:
        throw new ResourceNotFoundException(
            findTimeSetting.value() + " tanımsız. Sadece minute/second olabilir.");
    }
  }
}
